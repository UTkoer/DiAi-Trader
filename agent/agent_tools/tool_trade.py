...
from agent.general_tools import get_config_value, write_config_value, _load_runtime_env
from agent.price_tools import get_trade_price, get_latest_position, get_open_prices, get_yesterday_date, get_yesterday_open_and_close_price, get_yesterday_profit
...
def buy(symbol: str, amount: int) -> Dict[str, Any]:
    ...
    with _position_lock(signature):
        ...
        # This ID is used to ensure each operation has a unique identifier, Acquire lock for atomic read-modify-write on positions
        try:
        ...
        try:
            mode = get_config_value("BAR_MODE", "daily")  # daily / hourly
            this_symbol_price = get_trade_price(today_date, symbol, market, mode)
        except KeyError:
        ...
        if this_symbol_price is None:
            return {
            "terminal_error": f"Price data not available for {symbol} at {today_date}.",
            "symbol": symbol,
            "date": today_date,
            "market": market,}
        ...
        if cash_left < 0:
            ...
            log_path = get_config_value("MODEL_DATA_PATH", "./data/agent_data_astock/sse_50_day")
            position_file_path = os.path.join(log_path, signature, "position", "position.jsonl")
    ...
    with open(position_file_path, "r") as f:
        for line in f:
            ...
            try:
                ...
                if _extract_trade_day(record.get("date")) == _extract_trade_day(today_date):
...
if __name__ == "__main__":
    port = int(os.getenv("TRADE_HTTP_PORT", "8001")) # lsof -i :8001，kill PID
