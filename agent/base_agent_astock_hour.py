...
from agent.general_tools import (extract_conversation, extract_tool_messages,
                                 get_config_value, write_config_value)
from agent.price_tools import add_no_trade_record
...
class BaseAgentAStock_Hour(BaseAgentAStock):
    ...
    def get_trading_dates(self, init_date: str, end_date: str) -> List[str]:
        ...
        merged_file = Path(get_config_value("Ashare_DATA_PATH", "./data/a_stock_data/sse_50_hour/merged_hourly.jsonl"))
    ...
    async def run_trading_session(self, today_date: str) -> None:
        ...
        # timestamp 是完整时间，如 "2025-11-17 11:30:00"
        date_only = today_date.split(" ")[0]  # 提取 "2025-11-17"
        print(f"Creating agent for date: {date_only}, timestamp: {today_date}")
        ...
        self.agent = create_agent(
            self.model,
            tools=self.tools,
            system_prompt=get_agent_system_prompt_astock(date_only, self.signature, self.stock_symbols),
        )
