...
class ConfigLoader {
    ...
    async loadConfig() {
        ...
        try {
            ...
            console.log('【CONFIG.YAML 原始内容前300字符】：');
            console.log(yamlText.substring(0, 300));
            console.log('是否包含 markets.cn？ → ', yamlText.includes('markets') && yamlText.includes('cn'));
            return this.config;
        } catch (error) {
    ...
    getBenchmarkFile() {
        if (!this.config || !this.config.data) {
            return 'index_daily_sse_50.json';
        }
