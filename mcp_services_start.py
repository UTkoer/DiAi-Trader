...
class MCPServiceManager:
    def __init__(self):
        ...
        self.ports = {
            ...
            "trade": int(os.getenv("TRADE_HTTP_PORT", "8001")),
            "price": int(os.getenv("GETPRICE_HTTP_PORT", "8002")), 
            "price_online": int(os.getenv("GETPRICE_OnLINE_HTTP_PORT", "8003")),
        }

        # Service configurations
        mcp_server_dir = os.path.dirname(os.path.abspath(__file__))
        self.service_configs = {
            "math": {"script": os.path.join(mcp_server_dir, "agent/agent_tools/tool_math.py"), "name": "Math", "port": self.ports["math"]}, 
            "trade": {"script": os.path.join(mcp_server_dir, "agent/agent_tools/tool_trade.py"), "name": "TradeTools", "port": self.ports["trade"]},
            "price": {"script": os.path.join(mcp_server_dir, "agent/agent_tools/tool_get_price_local.py"), "name": "LocalPrices", "port": self.ports["price"]},
            "price_online": {"script": os.path.join(mcp_server_dir, "agent/agent_tools/tool_get_online_basic.py"), "name": "OnlinePrices", "port": self.ports["price_online"]},
        }
    ...
    def start_service(self, service_id, config):
        ...
        try:
            ...
            with open(log_file, "w") as f:
                process = subprocess.Popen([sys.executable, script_path], stdout=f, stderr=subprocess.STDOUT, cwd=os.getcwd())
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "status":
        ...
        # Startup mode, lsof -i :8001，kill PID
        manager = MCPServiceManager()
